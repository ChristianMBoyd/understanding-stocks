...
The Arbitrage Pricing Theory (APT) itself, seems like a generic conceptual framework for how returns ought to be related to intrinsic market factors, and {\it risk premiums} associated with those factors.  I'm having some difficulty making sense of the various representations in the article \cite{APT2026}, but will make my best-effort attempt to summarize my understanding in what follows.  Consider an asset\footnote{As far as I can currently tell, there's no particular reason to invoke the multi-dimensional/portfolio analysis stated in the Wikipedia article since all averages and formulas are per-asset anyway.} with rate of return $r$.  Then the APT states that the rates of return $r$ can be expressed in terms of $m\le n$ factors, $f_m$, 


\subsection{S. Ross - The arbitrage theory of capital asset pricing}

\note{[Todo]}: read \cite{Ross1976} after above\ldots