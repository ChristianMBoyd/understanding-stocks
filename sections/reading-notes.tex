...
\subsection{M. Sammon and J. Shim - Index rebalancing and stock market composition}
...
\subsubsection{Notes on the Introduction}
...
\subsubsection{Notes on Conceptual Framework}
...
\subsubsection{Notes on the portfolio analysis}
...
\subsubsection{Notes on designing a better index}

\note{[Todo]}: see if the authors can recoup this $\sim40$ bps under performance, or if they can construct rebalancing rules for an index fund that out-performs existing implementations by roughly 0.4\% in annualized terms.


\subsection{D. Ikenberry, et al - Market underreaction to open market share repurchases}
